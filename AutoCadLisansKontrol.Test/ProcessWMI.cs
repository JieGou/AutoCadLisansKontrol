    ...
    public class ProcessWMI
    {
        ...
        public void ExecuteRemoteProcessWMI(string remoteComputerName,string username,string password, string arguments, int WaitTimePerCommand)
        {
            ...
            try
            {
                ...
                using (ManagementEventWatcher wWatcher = new ManagementEventWatcher(manScope, q))
                {
                    bool stopped = false;

                    while (stopped == false)
                    {
                        using (ManagementBaseObject e = wWatcher.WaitForNextEvent())
                        {
                            
                            if (((ManagementBaseObject)e["TargetInstance"])["productID"].ToString() == this.ProcessId.ToString())
                            {
